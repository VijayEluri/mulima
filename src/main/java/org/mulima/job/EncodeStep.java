...
public class EncodeStep implements Step<Set<AudioFile>> {
  private static final Logger logger = LogManager.getLogger(EncodeStep.class);
   ...
   */
  public EncodeStep(
      MulimaService service, AudioFormat format, Set<AudioFile> inputs, File destDir) {
  ...
  public boolean execute() {
    ...
    for (var input : inputs) {
      ...
      var output = service.getFileService().createAudioFile(input, destDir, format);
      var codec = service.getToolService().getCodec(format);
      var result = codec.encode(input, output);
      if (result.isSuccess()) {
